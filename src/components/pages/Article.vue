<template>
  <div>
    <div class="container">
      ...
      <div v-if="response !== null && response.status === 200">
        <h4
          id="wrapper-pop"
        >Posted by {{response.data.author_info.showName}} &ndash; {{response.data.created_at}}</h4>
...
export default {
  ...
  async mounted() {
    ...
    topbar.show();
    let res = axios.get(
      "https://awooing.moe/api/v1/article/" + this.$route.params.id
    );
    await topbar.hide();
